---
title: "Nhật ký công việc Tuần 3"
date: 2026-07-11
weight: 3
chapter: false
pre: " <b> 1.3. </b> "
---

### Mục tiêu tuần 3:

* Thiết kế cơ sở dữ liệu DynamoDB và viết HTTP Client cho Unity.

### Các công việc triển khai trong tuần này (06/07/2026 - 11/07/2026):

| Ngày | Công việc | Ngày bắt đầu | Ngày hoàn thành |
| --- | --- | --- | --- |
| Thứ 2 | - Phân tích cấu trúc bảng và thiết lập Amazon DynamoDB trên AWS Console. | 06/07/2026 | 06/07/2026 |
| Thứ 3 | - Viết các class Repository (C#) để thao tác CRUD với dữ liệu game. | 07/07/2026 | 07/07/2026 |
| Thứ 4 | - Tìm hiểu mô hình kiến trúc UI Presenter và xây dựng Layout hiển thị trên Unity. | 08/07/2026 | 08/07/2026 |
| Thứ 5 | - Viết class ApiClient trong Unity dùng UnityWebRequest để gọi API bất đồng bộ. | 09/07/2026 | 09/07/2026 |
| Thứ 6 | - Xây dựng các thành phần UI xử lý trạng thái Loading (Spinner) và popup báo lỗi. | 10/07/2026 | 11/07/2026 |

### Kết quả đạt được:

* Lưu trữ và truy xuất thành công dữ liệu trò chơi từ DynamoDB.
* Unity Client có khả năng giao tiếp mượt mà với các RESTful API.
* Cải thiện trải nghiệm người dùng (UX) thông qua Loading UI và Error Handling.
