---
title : "References"
date : 2024-01-01
weight : 6
chapter : false
pre : " <b> 5.6. </b> "
---

#### Official Documentation & Helpful Resources

To deepen your understanding of the architecture, technologies, and patterns used in the **AI Dungeon RPG Adventure Game**, refer to the following official resources:

---

#### 1. AWS Generative AI & AWS Bedrock

- [AWS Bedrock Developer Guide](https://docs.aws.amazon.com/bedrock/latest/userguide/what-is-bedrock.html)
- [Anthropic Claude Models on AWS Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-claude.html)
- [Amazon Nova Models on AWS Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/titan-models.html)
- [Prompt Engineering Best Practices](https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-engineering-guidelines.html)

---

#### 2. AWS Serverless Compute & Database

- [AWS Lambda .NET 8 Runtime Guide](https://docs.aws.amazon.com/lambda/latest/dg/dotnet-image.html)
- [AWS Lambda Native AOT for .NET](https://docs.aws.amazon.com/lambda/latest/dg/dotnet-native-aot.html)
- [AWS Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html)
- [Amazon API Gateway REST API Documentation](https://docs.aws.amazon.com/apigateway/latest/developerguide/welcome.html)
- [Amazon Cognito User Pools & Identity Pools](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-identity-pools.html)
- [Amazon DynamoDB Best Practices & Single-Table Design](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/best-practices.html)

---

#### 3. Infrastructure as Code (AWS CDK C#)

- [AWS CDK Reference (.NET/C#)](https://docs.aws.amazon.com/cdk/api/v2/dotnet/api/index.html)
- [AWS CDK Concepts & Architecture](https://aws.amazon.com/cdk/)
- [AWS CDK Bootstrapping](https://docs.aws.amazon.com/cdk/v2/guide/bootstrapping.html)

---

#### 4. Unity Game Development

- [Unity UnityWebRequest & JSON Parsing](https://docs.unity3d.com/Manual/UnityWebRequest.html)
- [Unity ScriptableObject Guide](https://docs.unity3d.com/Manual/class-ScriptableObject.html)
- [Unity Async/Await & Coroutines Best Practices](https://docs.unity3d.com/Manual/Coroutines.html)
- [Unity Input System Package](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.7/manual/index.html)
- [TextMesh Pro Documentation](https://docs.unity3d.com/Packages/com.unity.textmeshpro@3.0/manual/index.html)
- [MVP Architecture Pattern in Unity](https://learn.unity.com/tutorial/building-a-udp-game)

---

#### 5. C# Full-Stack Monorepo & Shared Libraries

- [.NET Standard 2.1 Compatibility Guide](https://learn.microsoft.com/en-us/dotnet/standard/net-standard)
- [Monorepo Design Pattern for Unity & .NET](https://learn.microsoft.com/en-us/dotnet/standard/net-standard)
- [Microsoft Dependency Injection in .NET](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection)
- [System.Text.Json Source Generation (for Native AOT)](https://learn.microsoft.com/en-us/dotnet/standard/serialization/system-text-json/source-generation)
